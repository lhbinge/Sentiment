...
In general the series calculated with the new firm size weights are less volatile than the old series. The two-step procedure produces results that are very similar to those using the old weighting procedure with the new weights. The unweighted series are very similar to these series.
...
The SARB has two series available for total manufacturing activity, based on sales and volumes. StatsSA also has a series for total manufacturing volume. At the subsector level, only the StatsSA series are available. We compare the total activity results to the industrial production index. The correlations are relatively high for the total manufacturing sector. They are lower for the sub-sectors.

###Total Manufacturing Activity
Figure 23 illustrates the Total Building Activity series compared to the SARB and StatsSA references series. Table 10 reports that the contemporaneous correlations between the survey balances and the reference series are relatively high. This is confirmed by the cross-correlations, which show again that the survey balances lead the reference series by a few quarters. 

```{r figure23, echo=FALSE, warning=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Total Manufacturing compared to the reference series"}
#Reference series
...
Figure 24 illustrates the Total Building Activity series compared to the SARB and StatsSA references series. Table 11 reports that the contemporaneous correlations between the survey balances and the reference series are relatively high. This is confirmed by the cross-correlations, which show again that the survey balances lead the reference series by a few quarters. 

```{r figure24, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Manufacturing subsectors compared to the reference series"}
...
###Volatility
We can also compare the series in terms of volatility. Table 12 reports the volatilities of the series over time. The unweighted series are the least volatile. In addition, the unweighted series have the highest correlations with the reference series, i.e. the best tracking record. Nevertheless, we choose to follow the international norm of weighting the survey responses. This is because.
...
##Summary
So we have calculated lots of series. After some corrections the microdata seems to provide series that correspond very closely to the published series. We have calculated new series, using four weighting procedures, and compared them to the reference series. We chose the StatsSA series as reference series, as it is widely available for many of our series and has a relatively high correlation with the balances. The final series still have to be written to an Excel file. The updates can and should be fully automated.

\newpage
#The Trade Surveys 
In this section we examine the Manufacturing sector surveys and results: the Retail, Wholesale and Motor Vehicle surveys.

##Collating and Cleaning the Microdata
Ek het nou die mikrodata van 92Q2 tot 01Q1 wat jy uit die individuele files saamgestel het, by die historiese files (Retail_92Q2-17Q2.xlsx en Wholesale_19Q2-17Q2.xlsx) bygesit. Soos wat jy vir bou en vervaardiging gedoen het, moet jy eers toets of die resultate op grond van hierdie mikrodata en met die huidge faktore ooreenstem met die gepubliseerde data (op Quantec of sien BERdata_Retail.xlsx en BERdata_Wholesale.xlsx). Dit sal aantoon of daar êrens inkonsekwente sektor en faktor kodes ingesluip het. 
...
Similar to the Building Survey, in order to match the questions and responses in the different versions of the surveys, the surveys in the old records were compared to the current incarnations. We printed and marked the questions in the old (pre-2001) Retail surveys. We then matched them to the new survey questions. The old questionnaires have been uploaded to the Dropbox folder.
...
```{r table13, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
...
```{r figure26, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="The number of respondents by subsector (1992Q2-2017Q2)"}
BERplot <- BER.R
...
```{r figure27, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Retail subsectors compared to the published series"}
...
g1 <- g1 + ylab("Indicator") + xlab("")
g1 <- g1 + theme(legend.title=element_blank())
g1 <- g1 + ggtitle("Total Retail: Q3A") 
g1 <- g1 + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
g1 <- g1 + theme(legend.title=element_blank()) 
g1 <- g1 + scale_x_date(labels = date_format("%Y"),breaks = date_breaks("year"))
g1 <- g1 + theme(legend.position="none")
...
g2 <- g2 + theme(legend.title=element_blank())
g2 <- g2 + ggtitle("Retail Semi-Durable: Q3A") 
g2 <- g2 + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
g2 <- g2 + theme(legend.title=element_blank()) 
g2 <- g2 + scale_x_date(labels = date_format("%Y"),breaks = date_breaks("year"))
...
g4 <- g4 + theme(legend.title=element_blank())
g4 <- g4 + ggtitle("Retail Durable: Q3A") 
g4 <- g4 + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
g4 <- g4 + theme(legend.title=element_blank()) 
g4 <- g4 + scale_x_date(labels = date_format("%Y"),breaks = date_breaks("year"))
g4 <- g4 + theme(legend.position="bottom")
...
```{r figure28, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Retail regions compared to the published series"}
...
```{r figure29, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Alternative Retail series"}
...
```{r figure30, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Retail subsectors compared to the reference series"}
...
```{r figure31, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Total Retail compared to the reference series"}
...
```{r table15, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
...
colnames(retail_trade) <- c("Q2A","Q3A","Q2A_u","Q3A_u","Q2A_n","Q3A_n",
...
```{r figure32, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Retail cross-correlations to the reference series"}
Q2A <- retail_trade[,1]
...
```{r figure33, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Retail subsectors compared to the reference series"}
...
```{r table16, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
source("corstarsl.R")
...
```{r figure34, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Retail subsectors cross-correlations to the reference series"}
...
```{r table17, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
retail_trade <- cbind(Retail[,c("Q2A","Q3A")],Retail_u[,c("Q2A","Q3A")],
                      Retail_n[,c("Q2A","Q3A")],Retail_2[,c("Q2A","Q3A")],
                      ref_r[,c("Total_SARB","Total_StatsSA")]) 
colnames(retail_trade) <- c("Q2A","Q3A","Q2A_u","Q3A_u","Q2A_n","Q3A_n",
...
V <- t(as.data.frame(V))
row.names(V) <- c("Total Retail")
xt <- xtable(V, caption="Volatility of total retail series")
print(xt, "latex",comment=FALSE,scalebox = 0.9,
      caption.placement = getOption("xtable.caption.placement", "top"))
```

```{r table18, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
...
colnames(V) <- c("Q2A","Q3A","Q2A_n","Q3A_n","Q2A_2s","Q3A_2s")
...
```{r table19, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
...
```{r figure35, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Number of respondents in the wholesale sector (1992Q2-2017Q2)"}
BERplot <- BER.W
...
```{r figure36, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Wholesale subsectors compared to the published series"}
...
```{r figure37, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Wholesale regions compared to the published series"}
...
```{r figure39, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Number of Motor Vehicle respondents by region (1992Q2-2017Q2)"}
BERplot <- BER.V
...
```{r figure41, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Motor Vehicle regional series compared to the published series"}
...
```{r figure42, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Manufacturing subsectors compared to the reference series"}
...
```{r table22, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
...
source("corstarsl.R")
xt <- xtable(corstarsl(motor_trade)[5:6,1:4], caption="Motor Vehicle correlations")
print(xt, "latex",comment=FALSE, caption.placement = getOption("xtable.caption.placement", "top"), scalebox = 0.9)
```

```{r figure43, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Motor Vehicle regions compared to the reference series"}
...
xt <- xtable(xt,caption="Motor Vehicle regional correlations")
print(xt, "latex",comment=FALSE, caption.placement = getOption("xtable.caption.placement", "top"), scalebox = 0.9)
```
...
```{r table24, echo=FALSE, results='asis', warning=FALSE, message=FALSE, cache = TRUE}
...
.	Business services (8xxx excl. dié van real estate)
...
```{r figure44, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Number of respondents in the services sector (2005Q2-2017Q2)"}
BERplot <- BER.S
...
###Published Series
The results for the constraints (Q7-Q10) do not match. The results for Q1 do not match up perfectly.

```{r figure45, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Total Services series compared to the published series"}
#Total Services
...
```{r figure46, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Services subsectors compared to the published series"}
#Sales Volumes
...
```{r figure47, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE, fig.height=4.5, fig.width=7.5, fig.cap="Services regional series compared to the published series"}
...
#Verder
.	What subsector weights would provide the highest correlations with the reference 
o	PCA analysis?
o	Try Regression Analysis?

.	Include Latecomers in following quarter
.	Keep notes in report and update report
.	Convert survey dates in R - to aid updating
.	Can we weight all the survey indicators
.	Can we analyse moving averages

Laaste opmerkings
Die groot voordele wat dit vir ons inhou as jy al die sektore (ook dié waar ons geen faktore gebruik of gaan verander nie) se resultate met R herbereken, is dat ons dan lang tydreekse van verwagtinge bykry en ons die verwerking van al die sektore dan in die toekoms maklik na R kan oorskakel. Die enigste wat nou nog uitstaande is, is die CIBD se building en civils.

1.	Bereken afsonderlik die impak (a) van die insluiting van die "late comers" en (b) op die totaal as die sektore residensieel en nie-residensieel geweeg word. As die insluiting van "late comers" nie die resultate veel verander nie, kan ons by die huidige metode bly, sodat ons nie op gereelde basis die data hoef te hersien nie. Net so, as die sektor gewigte nie die grafiese verband verbeter en of volatiliteit verminder nie, bly ons by die huidige metode van geen gewigte in die bou sektor. 

1.	Ondersoek die seisoenaliteit van bou aktiwiteit vir die totaal, Wes-Kaap, Gauteng en KwaZulu-Natal. Sien Murray Pellissier se verslag SeasReport1205 in die Dropbox folder: Building

2.	Bepaal of daar ?? sikliese verskil tussen kontrakteurs en sub-kontrakteurs is? In praktyk behoort die aktiwiteit van sub-kontrakteurs dié van kontrakteurs te volg, maar bevestig die data dit?

3.	Ontwikkel ?? alternatiewe vertroue-indeks / barometer uit die gemiddelde verandering in "realised" en "expected business conditions" en vergelyk dit met die BEO se huidige vertroue-indeks (persentasie bevredig). 

4.	Ontleed en beoordeel die FNB/BER Building Contractor Confidence Index.


##Chaining
o	hoe die tydreeks van 70Q1-93Q1 aan die nuut berekende reekse vanaf 93Q2 gekoppel word. Stel voor dat ons nie die metode gebruik soos uitgespel op bl. 15 van die artikel SEE_2002_Trade Build Panel in die Dropbox folder: Total nie.

o	If the published series should are on the same scale, we should use the published series before 1993Q2 and the new series based on the microdata thereafter. 

o	To do: chain the series for those indices that are available. Add them to the presentation and the Excel file.

##When are movements "significant"?
.	Vergelyk die bou aktiwiteit reeks van residensieel, nie-residensieel, totale bou en drie vernaamste provinsies met die ooreenstemmende verwysingsreekse. Wys verband m.b.v. grafieke. Wat veroorsaak afwykings, bv. verskuiwings in die neutrale vlak en volatiliteit? Op die ou end wil ons weet hoe om die inligting meer gebruikersvriendelik grafies voor te stel en in die lig van die hoë volatiliteit te weet wanneer ?? spesifieke groot styging / daling in die resultate tussen kwartale uitsonderlik is. Byvoorbeeld: 

o	beweging rondom die langtermyn gemiddelde, mean-shifting (trek lang termyn gemiddelde af) of normalisering (trek gemiddelde af en deel deur standaardafwyking)
o	voeg ?? 3-periode einde bewegende gemiddelde by
o	bereken die afsnygrense waarbinne sê X% van alle absolute kwartaal-tot-kwartaal veranderings val

.	Nicolaas to compare them to average changes and confidence intervals, etc. A graphical illustration would be nice, perhaps as a confidence band. We can also scale the balances like the EC does with its series, to ease interpretation.
The EC monthly calculates EU and euro area aggregates on the basis of the national results and seasonally adjusts the balance series. The indicator is scaled to have a long-term mean of 100 and a standard deviation of 10; hence, values greater than 100 indicate above-average economic sentiment and vice versa. This indicator - drawing on confidence indicators in different sectors of the economy - can be considered very effective as a valuable tool to now-cast GDP before it is released by national accounts (see figure F10.2).

##Which questions to focus on
.	Adviseer watter 2-3 vrae ons op moet fokus in die "Opsomming van resultate aan die ekonome " en agter in die publikasie "Building & Construction " op grond van die mate waartoe hulle unieke inligting verskaf. Sien "Factor Analysis_Eon Smit_2012" in die Dropbox folder: Total

.	Nicolaas to report only a select few series. One could also use composite series.
