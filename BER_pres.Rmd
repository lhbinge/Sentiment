...
- Agents' perceptions of the current and expected future business climate.
- Degree of sentiment towards risk-taking by business for whatever reason.
...
+ associated with low confidence and heightened uncertainty 
...
- Qualitative surveys
- Current vs. forward-looking (or combination)
...
- Dispersion of responses from qualitative surveys

Relationship:

- Wait-and-see effect
- Other channels
- Usually anti-cyclical relationship
...
- 1,400 firms in construction & trade sectors (retail, wholesale & motor vehicles)
...
Component of SARB's official composite leading indicator 

Constructed from a question present in all surveys: 

- (Q1): *"Are prevailing business conditions: Satisfactory, Unsatisfactory?"* 
- Weighted percentage of respondents that responded *"Satisfactory"*
- Index of Current conditions or "Activity"

Weightings:

- Weighted by firm size and subsector weight
- Simple average of sectoral indicators
    + Sectoral indicators weighted by GDP share
...
## Confidence Indicators
Derived from questions present in all surveys:
...
Two measures capture a low level of predictability:
...
Dispersion in survey responses might simply be due to:

a) Different firms reacting differently to aggregate shocks (even with constant uncertainty). 
    + heatwave (ice cream vs. heaters)
b) Time variation in the heterogeneity of expectations (even with constant uncertainty).
    + periods when things are going well with Jaco but not with me

##Uncertainty Indicators
2) Cross-sectional dispersion in individual firm's expectation errors 

Survey at time *t* used to extract expectations for *t+1* (relative to *t-3*).

Survey at time *t+1* used to extract realisations (relative to *t-3*).  
...
Cross-sectional standard deviation of expectation errors: 
$$U_t = STD(Error_{t+1})$$
...
- Are they useful as leading/coincident indicators of the business cycle (more than BER BCI)
- Are they useful for now-casting and forecasting (even after fundamentals)
- Estimate in levels (LT impact)
...
## Further analysis
Puzzle: Why is the timing different?

- Misspecified VAR 
    + larger system & estimate in levels

- Noisy Indicators, or non-linear and asymmetric effects
    + dummies, smooth or unforecastable component
