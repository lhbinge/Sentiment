...
Figure 1 compares the RMB/BER Business Confidence Index to the confidence indicator derived from the same question. The results for the first activity indicator and the published index are very similar, even though the BER uses a slightly ad hoc weighting procedure. The difference is probably due to altered weightings and the BER's exclusion of latecomers. The same is true of the indicators for the respective sectors, not illustrated here.
...
```{r figure13, echo=FALSE, warning=FALSE, cache = TRUE, fig.height=4, fig.width=7.5, fig.cap="IRFs of real GDP growth to confidence and uncertainty"}
irf.y1 <- irf(var,impulse = c(name1,name2), response = name3, n.ahead = 12,runs = 1000, seed=12345) 
par(mfrow=c(1,2),mar=c(5,4,4,2), cex=0.5)
plot(irf.y1,plot.type = c("single"))
```

```{r figure14, echo=FALSE, warning=FALSE, cache = TRUE, fig.height=4, fig.width=7.5, fig.cap="IRFs of confidence and uncertainty to real GDP growth"}
